...
export interface CoverflowSlide {
  ...
  title?: string;
  subtitle?: string;
  /** Extra concrete lines describing what the reader is looking at. */
  details?: string[];
  /** Scale applied to the screenshot so its UI text stays legible in-card. */
  zoom?: number;
  /** CSS object-position / transform-origin of the informative region. */
  focus?: string;
  ...
  return (
    ...
    >
            ...
            {slides.map((slide, index) => (
              ...
              >
                <img
                  ...
                  style={{
                    objectPosition: slide.focus ?? "center top",
                    transform: slide.zoom ? `scale(${slide.zoom})` : undefined,
                    transformOrigin: slide.focus ?? "center top",
                  }}
                />
              </div>
            ))}
      ...
      {showCaption && active?.title && (
        <div
          key={selected}
          className="mx-auto mt-4 flex max-w-2xl flex-col items-center px-6 text-center duration-300 animate-in fade-in"
        >
          <p className="font-mono text-xl font-semibold tracking-tight text-foreground sm:text-2xl">
            {active.title}
          </p>
          {active.subtitle && (
            <p className="mt-2 text-base leading-7 text-foreground/80">{active.subtitle}</p>
          )}
