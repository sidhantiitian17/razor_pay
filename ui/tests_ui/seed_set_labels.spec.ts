...
test.describe("Seed Set Separation & Labels (Check 11.3, §4.4)", () => {
  test("dev seeds are visually separated and labelled 'tuning — not a claim'", async ({ page }) => {
    // Intercept Supabase endpoints to provide real sweep fixture with holdout, dev, and regression sets
    ...
    // Check Holdout distribution claim note
    await expect(
    ...
    // Verify Dev seeds count
    await expect(page.getByText("10 seeds present", { exact: true })).toBeVisible();
    ...
    // Verify Regression seed count
    await expect(page.getByText("1 seed present", { exact: true })).toBeVisible();
  });
});
